...
export default function Verify() {
	...
	useEffect(() => {
		...
		if (userDataState.verification?.status === "Pending" || verificationStatus === "Pending") {
			setUploaded(true);
		}

		if (userDataState.verification?.status === "Verified") {
	...
	return (
		<>
			<Breadcrumb pageName="Account Verification" />
			<div>
				{showCongratsUI && (
				...
				<div className="mb-20 text-center rounded-md border border-stroke  bg-white py-8 px-7.5 shadow-default dark:border-strokedark dark:bg-boxdark">
					...
					{verificationStatus === "Verified" && (
						<div className="flex flex-col text-center justify-center items-center text-xl mt-8 pb-8">
							<div className="text-8xl mb-5 text-meta-3">
								<MdVerified />
							</div>
							<br />
				...
				{!uploaded && (
					<form onSubmit={handleImageUpload}>
						...
						<div className="flex justify-center">
							<button
								className="flex justify-center md:w-1/2 mx-auto rounded bg-gold-orange py-3 px-6 font-medium text-gray hover:bg-opacity-95"
								type="submit"
							>
